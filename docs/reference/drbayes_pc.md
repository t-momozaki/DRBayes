...
#>   Treatment group: 111 | Control group: 89
...
#> Using 2000 posterior samples for analysis...
fit
#> Bayesian doubly robust causal inference via posterior coupling
#> 
#> Call:
#> drbayes_pc(outcome.formula = Y ~ A + X1 + X2 + A:X1, ps.formula = A ~ 
#>     X1 + X2 + X3, data = dat, control = drbayes_control(mc = 1500, 
#>     bn = 500), outcome.model = bayes_lm, ps.model = bayes_logit)
#> 
#> Outcome model:    gaussian family, identity link
#> Propensity score: logit link
#> Observations:     200 (111 treated, 89 control)
#> Posterior draws:  2000
#> 
#> Average treatment effect
#>         mean  2.5% 97.5%
#> pc     1.503 1.215 1.816
#> g.comp 1.501 1.211 1.805
#> 
...
#> 
#> Posterior coupling: lambda = -0.03, moment condition met, |mean B_n| = 0.00196
#> below the tolerance 0.0037, after 3 of 1000 steps.
#> 
#> Convergence diagnostics (worst over both models)
#>   Largest R-hat: 1.005 (outcome: X1)
#>   Smallest ESS:  1706 (propensity score: X3)
summary(fit)
#> Bayesian doubly robust causal inference via posterior coupling
#> 
#> Call:
...
#>         mean    sd  2.5%   50% 97.5%
#> pc     1.503 0.152 1.215 1.502 1.816
#> g.comp 1.501 0.155 1.211 1.503 1.805
#> 
...
#>   Tilting parameter lambda: -0.03
#>   Posterior mean of B_n:    0.00196
#>   Tolerance:                0.0037
#>   Steps taken:              3 of 1000
#>   Moment condition met:     yes
#> 
#> Convergence diagnostics
#>             model   parameter   mean     sd  rhat ess_bulk ess_tail mcse_mean
#>           outcome (Intercept)  1.114 0.1146 1.004     1998     1769   0.00257
#>           outcome           A  1.503 0.1552 1.002     1928     1928   0.00353
#>           outcome          X1  0.911 0.1136 1.005     2114     1852   0.00248
#>           outcome          X2  0.460 0.0769 1.001     2127     1932   0.00167
#>           outcome        A:X1  0.303 0.1590 1.003     2153     1779   0.00342
#>  propensity score (Intercept)  0.287 0.1570 0.999     2062     1938   0.00344
#>  propensity score          X1  0.568 0.1757 1.001     1922     1719   0.00401
#>  propensity score          X2 -0.660 0.1720 1.003     1774     1799   0.00409
#>  propensity score          X3  0.086 0.1642 1.001     1720     1706   0.00395
#> 
...
#>     X1 + X2 + X3, data = dat, control = drbayes_control(mc = 1500, 
#>     bn = 500), outcome.model = bayes_logit, ps.model = bayes_logit)
#> 
...
#> Average treatment effect
#>          mean    2.5%  97.5%
#> pc     0.0374 -0.0896 0.1738
#> g.comp 0.0595 -0.0712 0.1958
#> 
#> Report pc, the doubly robust estimand. g.comp is the untilted g-formula
#> posterior, which relies on the outcome model alone.
#> 
...
#> when that differs from the average over the population they were drawn from.
#> 
#> Posterior coupling: lambda = 0.24, moment condition met, |mean B_n| = 0.00129
#> below the tolerance 0.00163, after 24 of 1000 steps.
#> 
#> Convergence diagnostics (worst over both models)
#>   Largest R-hat: 1.002 (outcome: X2)
#>   Smallest ESS:  1715 (outcome: X1)
...
#> Auto-detected continuous outcome: using family='gaussian'
#> Generating posterior samples using provided models...
#> Using 1400 posterior samples for analysis...
#> Bayesian doubly robust causal inference via posterior coupling
#> 
#> Call:
#> drbayes_pc(outcome.formula = Y ~ A + log(abs(X1) + 1) + poly(X3, 
#>     2) + A:X1, ps.formula = A ~ X1 + X2 + X3, data = dat, control = drbayes_control(mc = 1000, 
#>     bn = 300), outcome.model = bayes_lm, ps.model = bayes_logit)
#> 
#> Outcome model:    gaussian family, identity link
#> Propensity score: logit link
#> Observations:     200 (111 treated, 89 control)
#> Posterior draws:  1400
#> 
#> Average treatment effect
#>         mean  2.5% 97.5%
#> pc     1.485 1.114 1.826
#> g.comp 1.485 1.114 1.826
#> 
#> Report pc, the doubly robust estimand. g.comp is the untilted g-formula
#> posterior, which relies on the outcome model alone.
#> 
...
#> when that differs from the average over the population they were drawn from.
#> 
#> Posterior coupling: lambda = 0, moment condition met, |mean B_n| = 0.00202
#> below the tolerance 0.00654, after 0 of 1000 steps.
#> 
#> Convergence diagnostics (worst over both models)
#>   Largest R-hat: 1.003 (outcome: poly(X3, 2)1)
#>   Smallest ESS:  1103 (outcome: poly(X3, 2)1)
...
#> Using 2500 posterior samples for analysis...
#> Bayesian doubly robust causal inference via posterior coupling
#> 
#> Call:
#> drbayes_pc(outcome.formula = Y ~ A + X1 + X2, ps.formula = A ~ 
#>     X1 + X2 + X3, data = dat, control = drbayes_control(bn = 1000), 
#>     outcome.samples = pool(otc), ps.samples = pool(ps))
#> 
#> Outcome model:    gaussian family, identity link
#> Propensity score: logit link
...
#> Average treatment effect
#>         mean  2.5% 97.5%
#> pc     1.516 1.177 1.895
#> g.comp 1.489 1.173 1.792
#> 
#> Report pc, the doubly robust estimand. g.comp is the untilted g-formula
#> posterior, which relies on the outcome model alone.
#> 
...
#> when that differs from the average over the population they were drawn from.
#> 
#> Posterior coupling: lambda = -1.21, moment condition met, |mean B_n| = 0.00324
#> below the tolerance 0.00344, after 121 of 1000 steps.
#> 
#> Convergence diagnostics (worst over both models)
#>   Largest R-hat: 1.003 (propensity score: X1)
#>   Smallest ESS:  2096 (propensity score: X1)
```
